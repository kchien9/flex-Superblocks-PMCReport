...
| Adoption Trend | Every entity gets its own thin line; the combined line stays bold/distinct. All shown at once — at ~8 entities one metric per line is fine (unlike the 3-metric chart below). |
...
- Bringing the 3 SMB-only trend slides into every Expansion deck regardless
  of segment.
- Dropping sparklines from the exec tile.
