...
const OpportunitySchema = z.object({
  ...
  Owner: z.object({
    Name: z.string(),
    Email: z.string(),
    UserRole: z.object({ Name: z.string() }).nullable(),
...
export default api({
  name: "GetLeaderboardClosedWon",
  description: "Fetches closed-won opportunities from Salesforce for leaderboard",
  ...
  async run(ctx, { period }) {
    ...
    const soql = `SELECT Account.Name, Owner.Name, Owner.Email, CloseDate, Flex_Units__c, StageName, Owner.UserRole.Name, Owner.Team_Name__c FROM Opportunity WHERE ${dateFilter} AND StageName = 'Closed Won' AND Test_Record__c = false AND RecordType.Name IN ('New Logo/Expansion', 'Deep SMB') AND Type IN ('New Logo', 'Expansion') AND (Owner.UserRole.Name LIKE '%Account Executive%' OR Owner.UserRole.Name LIKE '%Sales Manager%') ORDER BY Flex_Units__c DESC NULLS LAST LIMIT 500`;

    const results = await ctx.integrations.sf.query(soql, OpportunitySchema, {
      label: "Fetch closed won opps for leaderboard",
    });
